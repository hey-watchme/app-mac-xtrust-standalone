...
作成日: 2026-05-10 JST
最終更新: 2026-05-15 JST (LLM サーバー常駐化に伴う注記追加)

---

## 2026-05-15 注記: ライフサイクル変更との整合性

本ドキュメントが定める「サブプロセス分離による物理的クラッシュ防止」は、
LLM サーバー常駐化 (`llm-server-residency.md`) 後も**そのまま維持される**。

変わるのは MLX サブプロセスの**寿命**だけである:

- 旧: 1 リクエスト = 1 サブプロセス (使い捨て)
- 新: 1 サーバー長寿命プロセスを複数リクエストで共有

`MemoryPressureMonitor` が `.critical` で送る SIGKILL の経路は同一。
登録される PID が「短命の per-call PID」から「長寿命のサーバー PID」に
変わるだけで、kill すれば Metal/Unified Memory が即解放される性質は変わらない。

「Layer 1〜4」の防衛多層構造もそのまま適用される。Layer 2 のサブプロセス
分離は維持され、Layer 3 の OS memory pressure 監視も維持される。Layer 1
の起動前 sanity check は MLXModelServer 側に集約される (各 call 側からは
削除される)。
